...
export default function HomePage() {
  ...
  // ── Display / Apariencia state ────────────────────────────────────────────
  const [displayCatalog,  setDisplayCatalog]  = useState('hybrid');
  const [displayCategory, setDisplayCategory] = useState('auto');
  const [displaySaving,   setDisplaySaving]   = useState(false);

  // ── Welcome Menu state ────────────────────────────────────────────────
  type WmOption = { label: string; action: string; reply: string; url: string };
  const [wmEnabled,  setWmEnabled]  = useState(false);
  const [wmType,     setWmType]     = useState<'buttons'|'list'>('buttons');
  const [wmMessage,  setWmMessage]  = useState('');
  const [wmOptions,  setWmOptions]  = useState<WmOption[]>([]);
  ...
  const loadDisplay = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setDisplayCatalog(d.catalog_mode  ?? 'hybrid');
        setDisplayCategory(d.category_mode ?? 'auto');
      }
    } catch { /* silent */ }
  }, []);

  // ── Load + save welcome menu ───────────────────────────────────────────
  const loadWelcomeMenu = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/settings/welcome-menu`);
      if (res.ok) {
        const d = await res.json();
        const m = d.menu ?? {};
        setWmEnabled(m.enabled ?? false);
        setWmType(m.type ?? 'buttons');
        setWmMessage(m.message ?? '');
        setWmOptions((m.options ?? []).map((o: Record<string,string>) => ({
          label:  o.label  ?? '',
          action: o.action ?? 'catalog',
          reply:  o.reply  ?? '',
          url:    o.url    ?? '',
        })));
      }
  ...
  const saveWelcomeMenu = async (overrides?: Partial<{enabled:boolean; type:string; message:string; options:unknown[]}>) => {
    setWmSaving(true);
    try {
      const payload = {
        enabled: overrides?.enabled  ?? wmEnabled,
        type:    overrides?.type     ?? wmType,
        message: overrides?.message  ?? wmMessage,
        options: overrides?.options  ?? wmOptions,
      };
      const res = await fetch(`${API_BASE}/api/settings/welcome-menu`, {
        method:  'PATCH',
      ...
      if (res.ok) {
        showToast('success', '✅ Menú de bienvenida guardado');
      } else {
        const e = await res.json().catch(() => ({}));
        showToast('error', `❌ ${e.detail || 'Error al guardar'}`);
      }
  ...
  const addWmOption = () => {
    const max = wmType === 'list' ? 10 : 3;
    if (wmOptions.length >= max) {
      showToast('info', `⚠️ Máximo ${max} opciones para tipo ${wmType}`);
      return;
    }
    setWmOptions(prev => [...prev, { label: '', action: 'catalog', reply: '', url: '' }]);
  };

  const removeWmOption = (i: number) =>
    setWmOptions(prev => prev.filter((_, idx) => idx !== i));

  const updateWmOption = (i: number, field: string, val: string) =>
    setWmOptions(prev => prev.map((o, idx) => idx === i ? { ...o, [field]: val } : o));

  const handleDisplaySave = async (catalogMode: string, categoryMode: string) => {
    setDisplaySaving(true);
    try {
      const res = await fetch(`${API_BASE}/api/settings/display`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ catalog_mode: catalogMode, category_mode: categoryMode }),
      });
      if (res.ok) {
        setDisplayCatalog(catalogMode);
        setDisplayCategory(categoryMode);
        showToast('success', '✅ Apariencia guardada');
      } else {
        showToast('error', '❌ Error al guardar la apariencia');
      }
    } catch { showToast('error', '❌ Error de conexión'); }
  ...
  useEffect(() => {
    ...
    loadDisplay();
    loadWelcomeMenu();
  }, [loadVoiceNote, loadDisplay, loadWelcomeMenu]);
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 space-y-5">
        ...
        <div className="flex items-center justify-between gap-3 pb-2 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-blue-100 flex items-center justify-center text-lg flex-shrink-0">👋</div>
            <div>
              <h2 className="text-base font-bold text-gray-800">Men\u00fa de Bienvenida</h2>
              <p className="text-xs text-gray-400">Define qu\u00e9 botones o lista se muestran cuando el cliente saluda</p>
            </div>
          </div>
          {/* Master toggle */}
          <button
            ...
            className={`relative flex-shrink-0 w-12 h-6 rounded-full transition-colors duration-200 disabled:opacity-50 ${wmEnabled ? 'bg-blue-500' : 'bg-gray-300'}`}
          >
            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200 ${wmEnabled ? 'translate-x-7' : 'translate-x-1'}`} />
          </button>
        </div>

        {wmEnabled && (
          <div className="space-y-5">
            {/* Tipo: botones vs lista */}
            <div>
              <p className="text-xs font-semibold text-gray-600 mb-2">Tipo de men\u00fa</p>
              <div className="flex gap-2">
                {([
                  { v: 'buttons', icon: '🔘', label: 'Botones', hint: 'M\u00e1x. 3 opciones' },
                  { v: 'list',    icon: '📋', label: 'Lista',   hint: 'M\u00e1x. 10 opciones' },
                ] as const).map(opt => (
                  <button
                    key={opt.v}
                    type="button"
                    onClick={() => {
                      setWmType(opt.v);
                      if (opt.v === 'buttons' && wmOptions.length > 3)
                        setWmOptions(prev => prev.slice(0, 3));
                    }}
                    className={`flex-1 flex flex-col items-center gap-1 py-3 rounded-2xl border-2 text-xs font-semibold transition-all ${wmType === opt.v ? 'border-blue-400 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-blue-200'}`}
                  >
                    <span className="text-lg">{opt.icon}</span>
                    <span>{opt.label}</span>
                    <span className="text-gray-400 font-normal">{opt.hint}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5">Mensaje de bienvenida</label>
              <textarea
                rows={2}
                value={wmMessage}
                onChange={e => setWmMessage(e.target.value)}
                placeholder="ej: \u00a1Hola! \ud83d\udc4b Bienvenido a nuestra tienda. \u00bfEn qu\u00e9 te puedo ayudar?"
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white transition-all resize-none"
              />
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold text-gray-600">Opciones ({wmOptions.length}/{wmType === 'list' ? 10 : 3})</p>
                <button
                  type="button"
                  onClick={addWmOption}
                  disabled={wmOptions.length >= (wmType === 'list' ? 10 : 3)}
                  className="text-xs font-bold text-blue-600 hover:text-blue-800 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
                >
                  ＋ Agregar opci\u00f3n
                </button>
              </div>
              ...
              {wmOptions.length === 0 && (
                <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-2xl">
                  <p className="text-sm text-gray-400">Todav\u00eda no hay opciones.</p>
                  <p className="text-xs text-gray-300 mt-0.5">Haz clic en &ldquo;＋ Agregar opci\u00f3n&rdquo; para empezar.</p>
                </div>
              )}
                ...
                <div key={i} className="bg-gray-50 border border-gray-200 rounded-2xl p-3 space-y-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="text"
                      value={opt.label}
                      onChange={e => updateWmOption(i, 'label', e.target.value)}
                      placeholder="Texto del bot\u00f3n (ej: \ud83d\uded2 Ver cat\u00e1logo)"
                      maxLength={20}
                      className="flex-1 bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                    />
                    <button
                      type="button"
                      onClick={() => removeWmOption(i)}
                  ...
                  <div className="flex gap-2 pl-6">
                    {([
                      ...
                      <button
                        key={a.v}
                        type="button"
                        onClick={() => updateWmOption(i, 'action', a.v)}
                        className={`px-2.5 py-1 rounded-lg text-xs font-semibold border transition-all ${opt.action === a.v ? 'border-blue-400 bg-blue-100 text-blue-700' : 'border-gray-200 text-gray-400 hover:border-blue-200'}`}
                      >
                        {a.label}
                      </button>
                    ))}
                  ...
                  {/* Sub-campo si acción es texto */}
                  {opt.action === 'text' && (
                    <div className="pl-6">
                      <input
                        type="text"
                        value={opt.reply}
                        onChange={e => updateWmOption(i, 'reply', e.target.value)}
                        placeholder="Respuesta que enviar\u00e1 el bot..."
                        className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                      />
                    </div>
                  )}
                  {opt.action === 'url' && (
                    <div className="pl-6">
                      <input
                        type="url"
                        value={opt.url}
                        onChange={e => updateWmOption(i, 'url', e.target.value)}
                        placeholder="https://tu-tienda.com/catalogo"
                        className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                      />
                    </div>
                  )}
            ...
            {wmOptions.length > 0 && (
              <div className="bg-blue-50 border border-blue-100 rounded-2xl p-3 space-y-2">
                <p className="text-xs font-semibold text-blue-600">Vista previa en WhatsApp:</p>
                <p className="text-sm text-gray-700 leading-snug">{wmMessage || '(mensaje de bienvenida)'}</p>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {wmOptions.map((o, i) => (
                    <span key={i} className="px-3 py-1 bg-white border border-blue-200 rounded-full text-xs text-blue-700 font-medium shadow-sm">
                      {o.label || `Opci\u00f3n ${i + 1}`}
                    </span>
                  ))}
                </div>
            ...
            <button
              type="button"
              disabled={wmSaving}
              onClick={() => saveWelcomeMenu()}
              className="w-full py-3 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-sm font-bold rounded-2xl transition-all shadow-md shadow-blue-200 disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {wmSaving
                ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Guardando...</>
                : '💾 Guardar men\u00fa de bienvenida'}
            </button>
          </div>
        )}
        ...
        {!wmEnabled && (
          <p className="text-xs text-gray-400 text-center py-3">
            Activa el interruptor para crear tu men\u00fa personalizado. Mientras est\u00e9 desactivado, se mostrar\u00e1 el men\u00fa de categor\u00edas por defecto.
          </p>
        )}
      </div>

      {/* ── Card: Apariencia del Cat\u00e1logo ────────────────────────────────── */}
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 space-y-6">
        <div className="flex items-center justify-between gap-3 pb-2 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-emerald-100 flex items-center justify-center text-lg flex-shrink-0">🎨</div>
            <div>
              <h2 className="text-base font-bold text-gray-800">Apariencia del Catálogo</h2>
              <p className="text-xs text-gray-400">Elige cómo se presentan los productos y categorías en WhatsApp</p>
            </div>
          </div>
          {displaySaving && (
        ...
        {/* ── Categorías ──────────────────────────────────────────────────── */}
        <div className="space-y-3">
          <p className="text-sm font-bold text-gray-700">📂 Menú de Categorías</p>
          <p className="text-xs text-gray-400 -mt-2">¿Cómo aparecen las categorías cuando el cliente pide ver el catálogo?</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              { value: 'auto',    icon: '✨', name: 'Automático',       desc: 'Botones si ≤3, lista si ≤10, texto si más (recomendado)' },
              { value: 'buttons', icon: '🔘', name: 'Siempre botones',  desc: 'Máx. 3 opciones visibles — ideal para negocios con pocas categorías' },
              { value: 'list',    icon: '📋', name: 'Siempre lista',    desc: 'Hasta 10 opciones como menú desplegable de WhatsApp' },
              { value: 'text',    icon: '📝', name: 'Siempre texto',    desc: 'Lista de viñetas — sin límite de categorías' },
            ].map((opt) => {
              const isActive = displayCategory === opt.value;
              return (
                <button
                  key={opt.value}
                  type="button"
                  disabled={displaySaving}
                  onClick={() => handleDisplaySave(displayCatalog, opt.value)}
                  className={`flex items-start gap-3 p-3 rounded-2xl border-2 text-left transition-all duration-150 disabled:opacity-50 ${
                    isActive
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold leading-tight ${isActive ? 'text-emerald-700' : 'text-gray-700'}`}>
                      {opt.name}
                      {isActive && <span className="ml-2 text-xs font-bold bg-emerald-200 text-emerald-800 px-1.5 py-0.5 rounded-full">activo</span>}
                    </p>
        ...
        <div className="space-y-3 pt-4 border-t border-gray-100">
          <p className="text-sm font-bold text-gray-700">🛍️ Vista de Productos</p>
          <p className="text-xs text-gray-400 -mt-2">¿Cómo se muestran los productos en los resultados de búsqueda?</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              { value: 'hybrid',      icon: '⚡', name: 'Híbrido',          desc: 'Carrusel para productos con foto, texto para el resto (recomendado)' },
              { value: 'carousel',    icon: '🎠', name: 'Carrusel visual',  desc: 'Tarjetas con foto, precio y botón — requiere imágenes en el producto' },
              { value: 'list',        icon: '📋', name: 'Lista interactiva', desc: 'Hasta 10 productos en menú seleccionable — ideal para muchas variantes' },
              { value: 'text',        icon: '📝', name: 'Texto simple',     desc: 'Listado de viñetas — funciona siempre, sin límites de cantidad' },
            ].map((opt) => {
              const isActive = displayCatalog === opt.value;
              return (
                <button
                  key={opt.value}
                  type="button"
                  disabled={displaySaving}
                  onClick={() => handleDisplaySave(opt.value, displayCategory)}
                  className={`flex items-start gap-3 p-3 rounded-2xl border-2 text-left transition-all duration-150 disabled:opacity-50 ${
                    isActive
                      ? 'border-emerald-400 bg-emerald-50'
                      : 'border-gray-100 bg-gray-50 hover:border-emerald-200 hover:bg-emerald-50/40'
